...
Top_states=data_merged.groupby(['Month','Weather_Condition', 'State'])["ID"].count()
...
layout = html.Div([
    dbc.Container([
       ...
       dcc.Dropdown(
        ...
        dbc.Row([
            dbc.Col(dbc.Card(html.H3(children='State vs Number of Accidents Monthly',
                                     className="text-center text-light bg-dark"), body=True, color="dark")
            , className="mt-4 mb-4")
        ]),
        dcc.Graph(id='State_accidents_month', figure ={}),
...
@app.callback(
    Output('State_accidents_month', 'figure'),
    Input('Weather_Condition_Dropdown', 'value')
    #Input('Weather_Condition_Dropdown', 'value2')
)


def update_output(value):
    data_States = Top_states_accident[Top_states_accident['Weather_Condition'] == value]
    barchart1 = px.bar(
        data_frame=data_States[['State','Accident_count']],
        x='State',
        y='Accident_count'
